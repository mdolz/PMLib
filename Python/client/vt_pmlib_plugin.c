 ...
 */
static int strwldcmp(char *** parts, int * num_params, char * name)
{
    ...
    *parts= (char**) malloc(sizeof(char*));
	...
	(*parts)[ num_parts - 1 ]= pos;
        *parts= (char**) realloc(*parts, (num_parts+1)*sizeof(char*));
        pos= strstr(pos, "_");
        if ( pos != NULL )
        {
            *pos='\0'; pos++;
...
{
    int count, selected= 0, status= 0, num_params= 0;
    char **params;
    ...
    if ( !strwldcmp(&params, &num_params, event_name) )
    { 
        ...
        if ( ! status )
	{
            ...
            if (num_params == 4)
            {
                return_values[selected-1].name= malloc( (7 + strlen(params[3])) * sizeof(char) );
               // sprintf(return_values[selected-1].name, "Power: %s", params[3]);
                sprintf(return_values[selected-1].name, "%s", params[3]);
            }
            else
            {
                return_values[selected-1].name= malloc( (7 + strlen(event_name)) * sizeof(char) );
               // sprintf(return_values[selected-1].name, "Power: %s", event_name);
                sprintf(return_values[selected-1].name, "%s", event_name);
            }
            //return_values[selected-1].unit = strdup("Watts");
            return_values[selected-1].unit = strdup("");
            ...
            if (num_params == 4)
            {
                counters[num_counters-1].name= malloc( (7 + strlen(params[3])) * sizeof(char) );
               // sprintf(counters[num_counters-1].name, "Power: %s", params[3]);
                sprintf(counters[num_counters-1].name, "%s", params[3]);
            }
            else
            {
                counters[num_counters-1].name= malloc( (7 + strlen(event_name)) * sizeof(char) );
              //  sprintf(counters[num_counters-1].name, "Power: %s", event_name);
                sprintf(counters[num_counters-1].name, "%s", event_name);
            }
...
uint64_t get_all_values(int32_t counter, vt_plugin_cntr_timevalue ** result_vector)
{
    ...
    else if ( count > 0 )
    {
        ...
        str= malloc( ( strlen(counters[counter].name) + 5 ) * sizeof(char) );
        //sprintf(str, "%s.pwr", counters[counter].name);
        //pm_print_data_text(str, counters[counter].counter, counters[counter].lines_mask, 0);
	free(str);
    }
...
int32_t enable_counter(int32_t counter)
{
    ...
    printf("Start pmlib counter %d\n", counter); fflush(0);
    #endif
    int res= pm_start_counter(&counters[counter].counter);
    counters[counter].timer_start= vt_timer_function();
    return res;
}
*/
