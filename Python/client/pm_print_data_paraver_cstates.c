...
//  double	sum;
  long double t, tot_time, inc_time, value;
...
//  printf("inc_time: %0Lf\n", inc_time);
 ...
 printf("t: %0Lf\n", t);
 value_ant=(long double *)malloc( 8*sizeof(long double));
 for (i=0; i<8; i++){
	value_ant[i]=0.0;
 }
 for( s= init; s < last; s++ )
...
//			printf("ind_lines[%d]: %d, cpu: %d\n", j, ind_lines[j], cpu);
			if(separate){
				...
				if(pm_counter.measures->energy.watts[i+interval*ind_print[j]]!=0.0){
					value=(inc_time*pm_counter.measures->energy.watts[i+interval*ind_print[j]])/100;
					val_event=event-900000010;
				//	printf("value: %0Lf, event: % ld, t: %0Lf", value, val_event, t);
					fprintf(file_data, "2:0:1:1:%d:%0Lf:90000000:%d\n", cpu+1, value_ant[cpu]+value, val_event );
					value_ant[cpu]+=value;
				}
			}
